...
describe('NotificationsPage', () => {
  ...
  it('updates the sidebar unread badge immediately when a notification is marked read (finding-6)', async () => {
    const user = userEvent.setup();
    let read = false;
    vi.stubGlobal(
      'fetch',
    ...
    // Sidebar badge starts showing the stale unread count from the shared context.
    expect(await screen.findByLabelText(/1 unread notification/i)).toBeTruthy();

    await user.click(await screen.findByRole('button', { name: /mark read/i }));

    // Badge must disappear immediately — it must not wait for its own separate poll.
    await waitFor(() => expect(screen.queryByLabelText(/unread notification/i)).toBeNull());
  });
});
